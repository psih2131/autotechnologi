<template>
  <section class="about-reviews-section">
    <div class="container">
      <div class="about-reviews">
        <div class="about-reviews__head">
          <h2 class="about-reviews__title">Отзывы учеников</h2>
          <p class="about-reviews__subtitle">
            Реальные ситуации водителей, специалистов и организаций, которые прошли обучение и получили документы.
          </p>
        </div>

        <div class="about-reviews__videos">
          <div
            v-for="(video, idx) in reviewVideos"
            :key="idx"
            class="about-reviews__video-item"
          >
            <iframe
              :src="video.src"
              :title="video.title"
              class="about-reviews__video-iframe"
              frameborder="0"
        ...
        <div class="about-reviews__text-slider">
          <div class="about-reviews__nav-row">
            <button
              type="button"
              class="about-reviews__nav about-reviews__nav--prev"
            ...
            <ClientOnly>
              <swiper-container ref="containerRef" :init="false" class="about-reviews__swiper">
                <swiper-slide
                  v-for="(slide, idx) in reviewSlides"
                  :key="idx"
                  class="about-reviews__slide"
                >
                  <article
                    v-for="(review, rIdx) in slide"
                    :key="rIdx"
                    class="about-reviews__card"
                  >
                    <p class="about-reviews__text">{{ review.text }}</p>
                    <div class="about-reviews__author">
                      <img :src="review.avatar" :alt="review.name" class="about-reviews__avatar">
                      <div>
                        <p class="about-reviews__author-name">{{ review.name }}</p>
                        <p class="about-reviews__author-city">{{ review.city }}</p>
                      </div>
                    </div>
                  </article>
                </swiper-slide>
          ...
          <div class="about-reviews__pagination">
            <button
              v-for="(_, idx) in reviewSlides"
              :key="idx"
              type="button"
              class="about-reviews__dot"
              :class="{ 'about-reviews__dot--active': activeSlide === idx }"
...
const reviewSlides = [
  [
    {
      text: 'Всё было изложено простым языком, так понятно и доступно, а главное Вы дали нам именно то, что действительно необходимо в ежедневной работе.',
      name: 'Олег Кукушкин',
      city: 'Москва',
      avatar: avatar2,
    },
    {
      text: 'В целом все не архи сложно и довольно удобно для человека, который замотивирован и умеет планировать свое время.',
  ...
  [
    {
      text: 'Обучение прошло в удобном формате, материал структурирован и понятен. Особенно ценно, что можно проходить курс в своём темпе.',
      name: 'Андрей Петров',
      city: 'Челябинск',
      avatar: avatar1,
    },
