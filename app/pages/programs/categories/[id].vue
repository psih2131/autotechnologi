      ...
      <section class="programs-section">
        <div class="container">
          ...
          <nav class="programs-main__breadcrumbs">
                ...
                <NuxtLink to="/programs">Все курсы</NuxtLink>
                <span v-if="currentCategory.title" class="programs-main__breadcrumbs-sep">/</span>
                <span v-if="currentCategory.title">{{ currentCategory.title }}</span>
              </nav>
          ...
          <div class="programs-section__layout">
                  ...
                  <ul class="programs-sidebar__nav-claster-list-items">
                    ...
                    >
                      <NuxtLink
                        ...
                        :class="{ active: category.slug === categoryId }"
                      >
            ...
            <div class="programs-main">
              ...
              <div class="programs-main__head">
                <h1 class="programs-main__title">{{ currentCategory.title }}</h1>
                <p v-if="currentCategory.subtitle" class="programs-main__description">
              ...
              <div v-else class="programs-main__empty">
                ...
                <p class="programs-main__empty-text">
                  В направлении «{{ currentCategory.title }}» пока нет доступных курсов.
                  Выберите другое направление или посмотрите все программы.
                </p>
                <NuxtLink to="/programs" class="programs-main__empty-link">Все курсы</NuxtLink>
              </div>
            </div>
          </div>
        </div>
...
const { data: categoryData } = await useAsyncData(
  () => `programs-category-${categoryId.value}`,
  async () => {
    if (!categoryId.value) return null

    const params = new URLSearchParams({
      'filters[slug][$eq]': categoryId.value,
      'populate[Seo][populate][shareImage]': 'true',
      'populate[Seo][populate][twitterImage]': 'true',
    })
...
const currentCategory = computed(() => ({
  ...
  subtitle: categoryData.value?.subtitle || '',
}))
...
const allPrograms = computed(() =>
  (programsData.value || [])
    .filter((item) => item?.slug && item?.title)
    .map(mapProgram)
    .filter((item) => item.categorySlug === categoryId.value),
)
...
const banner = computed(() => pageData.value?.data?.banner_section ?? null)

const seo = computed(() => categoryData.value?.Seo ?? null)

useSeoMeta({
  title: () => seo.value?.metaTitle || currentCategory.value?.title || undefined,
